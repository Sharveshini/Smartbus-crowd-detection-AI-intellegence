// ================================================================
// SMARTBUS NEO - Next-Gen AI-Powered Public Transport Platform
// Features: AI Prediction, Seat Availability, Smart Routing,
//           Women Safety, AI Assistant, Voice Support
// ================================================================
...
// ─── AI Crowd Prediction Engine (15-30 min ahead) ───
function predictCrowdAdvanced(busId, currentPax) {
  const history = db.prepare('SELECT * FROM gps_logs WHERE bus_id = ? ORDER BY timestamp DESC LIMIT 30').all(busId);
  const predictions = db.prepare('SELECT * FROM crowd_predictions WHERE bus_id = ? ORDER BY predicted_at DESC LIMIT 3').all(busId);
  
  let trend = 'stable', confidence = 0.5, predictedIn15 = currentPax, predictedIn30 = currentPax;
  
  if (history.length >= 5) {
    const recent = history.slice(0, 10);
    const changes = [];
    for (let i = 1; i < recent.length; i++) {
      const c = (recent[i-1].passengers_count || 0) - (recent[i].passengers_count || 0);
      if (!isNaN(c)) changes.push(c);
    }
    const avgChange = changes.length > 0 ? changes.reduce((a, b) => a + b, 0) / changes.length : 0;
    const stdDev = Math.sqrt(changes.reduce((s, v) => s + (v - avgChange) ** 2, 0) / changes.length) || 1;
    
    // Time-series forecasting: predict 15 min and 30 min ahead
    const hourFactor = new Date().getHours();
    const peakMultiplier = (hourFactor >= 8 && hourFactor <= 10) || (hourFactor >= 17 && hourFactor <= 19) ? 1.3 : 0.9;
    
    predictedIn15 = Math.round(clamp(currentPax + avgChange * 3 * peakMultiplier, 0, CAPACITY));
    predictedIn30 = Math.round(clamp(currentPax + avgChange * 6 * peakMultiplier, 0, CAPACITY));
    
    if (avgChange > 2) trend = 'increasing';
    else if (avgChange < -2) trend = 'decreasing';
    confidence = Math.min(0.92, 0.4 + (history.length * 0.02) + (stdDev < 5 ? 0.2 : 0));
  }
  
  // Zone distribution
  const doorPax = Math.round(predictedIn15 * rand(0.15, 0.35));
  const middlePax = Math.round(predictedIn15 * rand(0.3, 0.5));
  const backPax = predictedIn15 - doorPax - middlePax;
  
  // Save prediction
  const now = new Date();
  const future15 = new Date(now.getTime() + 15 * 60000).toISOString();
  const future30 = new Date(now.getTime() + 30 * 60000).toISOString();
  
  db.prepare('INSERT INTO crowd_predictions (bus_id, prediction_for, predicted_level, predicted_count, confidence, trend, zone_door, zone_middle, zone_back) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
    .run(busId, future15, getLevel(predictedIn15), predictedIn15, confidence, trend, doorPax, middlePax, backPax);
  ...
  return {
    ...
    trend, confidence,
    zones: { door: doorPax, middle: middlePax, back: backPax },
    recommendation: predictedIn15 > 40 ? 'Consider waiting for next bus or taking alternate route' : 'Comfortable to board'
  };
}

// ─── AI Smart Route Recommendation Engine ───
function smartRouteRecommend(fromStopId, toStopId, preferences = {}) {
  const mode = preferences.mode || 'balanced'; // fastest, cheapest, least_crowded, safest, balanced
  const graph = {};
  const allStops = stmts.getAllStops.all();
  const routes = stmts.getAllRoutes.all();
  const buses = stmts.getAllBuses.all();
  
  allStops.forEach(s => graph[s.id] = []);
  
  for (const route of routes) {
    const stops = stmts.getRouteStops.all(route.id);
    for (let i = 0; i < stops.length - 1; i++) {
      const from = stops[i], to = stops[i + 1];
      const weight = to.time_from_prev_min || 2;
      const crowdData = buses.filter(b => b.route_name && b.route_name.includes(route.name));
      const avgCrowd = crowdData.length > 0 ? crowdData.reduce((s, b) => s + (b.passengers_count || 0), 0) / crowdData.length : 20;
      const crowdFactor = avgCrowd / CAPACITY;
      
      // Multi-criteria weight calculation
      let timeWeight = weight;
      let costWeight = (route.fare_base || 20) / 50;
      let crowdWeight = crowdFactor * 10;
      let safetyWeight = (5 - (route.safety_rating || 4.5)) * 2;
      let totalWeight = 0;
      
      switch(mode) {
        case 'fastest': totalWeight = timeWeight; break;
        case 'cheapest': totalWeight = costWeight; break;
        case 'least_crowded': totalWeight = crowdWeight; break;
        case 'safest': totalWeight = safetyWeight; break;
        default: totalWeight = (timeWeight * 0.3 + costWeight * 0.2 + crowdWeight * 0.3 + safetyWeight * 0.2);
      }
      
      graph[from.stop_id].push({ to: to.stop_id, weight: totalWeight, timeWeight, costWeight, crowdWeight, safetyWeight,
        routeId: route.id, routeName: route.name, routeNumber: route.number, duration: weight, fare: route.fare_base || 20,
        safetyRating: route.safety_rating || 4.5, crowdLevel: avgCrowd, seatsAvail: CAPACITY - avgCrowd });
      graph[to.stop_id].push({ to: from.stop_id, weight: totalWeight, timeWeight, costWeight, crowdWeight, safetyWeight,
        routeId: route.id, routeName: route.name, routeNumber: route.number, duration: weight, fare: route.fare_base || 20,
        safetyRating: route.safety_rating || 4.5, crowdLevel: avgCrowd, seatsAvail: CAPACITY - avgCrowd });
    }
  }

  // Dijkstra with multiple criteria
  function dijkstraMulti(graph, start, end, weightKey) {
    const distances = {}, previous = {};
    Object.keys(graph).forEach(n => { distances[n] = Infinity; previous[n] = null; });
    distances[start] = 0;
    const pq = new Map(); pq.set(start, 0);
    
    while (pq.size > 0) {
      let minNode = null, minDist = Infinity;
      for (const [node, dist] of pq) { if (dist < minDist) { minDist = dist; minNode = node; } }
      pq.delete(minNode);
      if (minNode === end) break;
      for (const neighbor of graph[minNode] || []) {
        const alt = distances[minNode] + (neighbor[weightKey] || neighbor.weight);
        if (alt < distances[neighbor.to]) {
          distances[neighbor.to] = alt;
          previous[neighbor.to] = { node: minNode, ...neighbor };
  ...
  const modes = [
    { key: 'timeWeight', name: 'Fastest', icon: '⚡' },
    { key: 'costWeight', name: 'Cheapest', icon: '💰' },
    { key: 'crowdWeight', name: 'Least Crowded', icon: '👥' },
    { key: 'safetyWeight', name: 'Safest', icon: '🛡️' },
  ];

  const results = modes.map(m => {
    const r = dijkstraMulti(graph, fromStopId, toStopId, m.key);
    if (!r) return null;
    const totalTime = r.path.reduce((s, p) => s + (p.duration || 2), 0);
    const totalFare = r.path.reduce((s, p) => s + (p.fare || 20), 0);
    const avgCrowd = r.path.length > 0 ? r.path.reduce((s, p) => s + (p.crowdLevel || 20), 0) / r.path.length : 20;
    const avgSafety = r.path.length > 0 ? r.path.reduce((s, p) => s + (p.safetyRating || 4.5), 0) / r.path.length : 4.5;
    const seats = Math.min(...r.path.map(p => p.seatsAvail || 60));
    return {
      mode: m.name, icon: m.icon, totalTime, totalFare, avgCrowd: Math.round(avgCrowd),
      seatsAvailable: Math.round(seats), safetyRating: Math.round(avgSafety * 10) / 10,
      transfers: r.path.length - 1,
      legs: r.path.map(p => ({
        routeName: p.routeName || 'Unknown', routeNumber: p.routeNumber || '',
        fromStop: p.from_stop, toStop: p.to, duration: p.duration || 2,
        fare: p.fare || 20, crowdLevel: Math.round(p.crowdLevel || 20),
        seatsAvail: Math.round(p.seatsAvail || 60)
      }))
    };
  }).filter(Boolean);

  // Sort by preference mode
  const modeOrder = { fastest: 0, cheapest: 1, least_crowded: 2, safest: 3, balanced: 0 };
  const priority = modeOrder[mode] || 0;
  
  return {
    recommendations: results,
    bestMatch: results[priority] || results[0],
    trafficAware: new Date().getHours() >= 9 && new Date().getHours() <= 11 ? 'moderate' : 'light',
    weatherAware: 'clear'
  };
}

// ─── AI Travel Assistant (Chatbot) ───
function aiAssistant(userMessage, userId, language = 'en') {
  const msg = userMessage.toLowerCase();
  let intent = 'general', response = '', context = {};
  
  // Intent Detection
  if (msg.includes('crowd') || msg.includes('busy') || msg.includes('packed') || msg.includes('full')) {
    intent = 'crowd_query';
    const bus = stmts.getAllBuses.all().sort((a, b) => (b.passengers_count || 0) - (a.passengers_count || 0))[0];
    response = language === 'ta' 
      ? `தற்போது ${bus?.route_name || 'பேருந்து'} பாதையில் அதிக நெரிசல் உள்ளது. ${bus?.passengers_count || 0} பேர் பயணிக்கின்றனர். மாற்று பாதையை பரிந்துரைக்கிறோம்.`
      : `🚍 Currently, ${bus?.route_name || 'the bus route'} has high crowding with ${bus?.passengers_count || 0} passengers. I recommend an alternate route.`;
    context = { busId: bus?.id, routeName: bus?.route_name };
  }
  else if (msg.includes('seat') || msg.includes('empty') || msg.includes('space') || msg.includes('available')) {
    intent = 'seat_query';
    const bus = stmts.getAllBuses.all().sort((a, b) => (b.seats_available || 0) - (a.seats_available || 0))[0];
    response = language === 'ta'
      ? `பேருந்து ${bus?.registration || 'அடையாளம்'} இல் ${bus?.seats_available || 0} இருக்கைகள் காலியாக உள்ளன.`
      : `✅ Bus ${bus?.registration || 'unknown'} has ${bus?.seats_available || 0} seats available!`;
    context = { busId: bus?.id, seats: bus?.seats_available };
  }
  else if (msg.includes('route') || msg.includes('plan') || msg.includes('go') || msg.includes('travel')) {
    intent = 'route_planning';
    const stops = stmts.getAllStops.all();
    const from = stops.find(s => msg.includes(s.name.toLowerCase().slice(0, 6))) || stops[0];
    const to = stops.find(s => s.id !== from.id && msg.includes(s.name.toLowerCase().slice(0, 6))) || stops[stops.length - 1];
    const route = smartRouteRecommend(from.id, to.id, { mode: 'balanced' });
    response = language === 'ta'
      ? `${from.name} இலிருந்து ${to.name} வரை ${route.bestMatch.totalTime} நிமிடங்கள், ₹${route.bestMatch.totalFare} கட்டணம். ${route.bestMatch.transfers} மாற்றங்கள்.`
      : `🗺️ From ${from.name} to ${to.name}: ${route.bestMatch.totalTime} min, ₹${route.bestMatch.totalFare}, ${route.bestMatch.transfers} transfer(s). Best option: ${route.bestMatch.mode} route.`;
    context = { fromStop: from.name, toStop: to.name, route };
  }
  else if (msg.includes('safe') || msg.includes('women') || msg.includes('safety') || msg.includes('emergency')) {
    intent = 'safety';
    response = language === 'ta'
      ? `🔒 பெண்கள் பாதுகாப்பு முறை செயல்படுத்தப்பட்டது. உங்கள் பயணத்தை நிகழ்நேரத்தில் பகிரலாம். SOS எச்சரிக்கை அனுப்ப கிளிக் செய்யவும்.`
      : `🛡️ Women Safety Mode is available! Features:\n📍 Live journey sharing\n🚨 SOS emergency alert\n👥 Emergency contacts\n📱 Real-time tracking`;
    context = { safetyMode: true };
  }
  else if (msg.includes('help') || msg.includes('hi') || msg.includes('hello') || msg.includes('hey')) {
    intent = 'greeting';
    response = language === 'ta'
      ? `வணக்கம்! 👋 நான் SmartBus AI உதவியாளர். நெரிசல் தகவல், இருக்கை கிடைக்கும் தன்மை, பயண திட்டமிடல் மற்றும் பாதுகாப்பு அம்சங்களுக்கு உதவுகிறேன்.`
      : `👋 Hello! I'm SmartBus AI Assistant. I can help with:\n• Real-time crowd levels & predictions\n• Seat availability\n• Route planning & recommendations\n• Women safety mode\n• Emergency SOS`;
    context = { capabilities: ['crowd', 'seats', 'routes', 'safety', 'sos'] };
  }
  else if (msg.includes('tamil') || msg.includes('ta')) {
    language = 'ta';
    response = '✅ மொழி தமிழுக்கு மாற்றப்பட்டது. எப்படி உதவ முடியும்?';
    context = { language: 'ta' };
  }
  else if (msg.includes('english') || msg.includes('en')) {
    language = 'en';
    response = '✅ Language switched to English. How can I help you?';
    context = { language: 'en' };
  }
  else if (msg.includes('reward') || msg.includes('points') || msg.includes('coin')) {
    intent = 'rewards';
    const user = stmts.getUserById.get(userId);
    response = language === 'ta'
      ? `உங்கள் வெகுமதி புள்ளிகள்: ${user?.total_reward_points || 0}. பயணம் செய்து மேலும் புள்ளிகள் பெறுங்கள்!`
      : `⭐ Your reward points: ${user?.total_reward_points || 0}. Earn more by traveling!`;
    context = { points: user?.total_reward_points || 0 };
  }
  else {
    response = language === 'ta'
      ? `மன்னிக்கவும், உங்கள் கேள்வியை புரிந்துகொள்ளவில்லை. நெரிசல், இருக்கைகள், பாதை திட்டமிடல், பாதுகாப்பு அல்லது வெகுமதிகள் பற்றி கேட்கலாம்.`
      : `🤔 I didn't understand that. Try asking about:\n• Crowd levels & predictions\n• Seat availability\n• Route planning\n• Safety features\n• Rewards & points`;
  }

  // Save conversation
  const cid = uuid();
  stmts.saveChatMessage.run(cid, userId, userMessage, response, intent, JSON.stringify(context));
  
  return { intent, response, context, language };
}
...
// ─── API: AI CROWD PREDICTION ───
app.get('/api/ai/predict/:busId', (req, res) => {
  const bus = stmts.getBusById.get(req.params.busId);
  if (!bus) return res.status(404).json({ error: 'Bus not found' });
  const prediction = predictCrowdAdvanced(req.params.busId, bus.passengers_count || 0);
  res.json(prediction);
});

app.get('/api/ai/predictions', (req, res) => {
  const buses = stmts.getAllBuses.all();
  const predictions = buses.map(b => ({
    busId: b.id, registration: b.registration, route_name: b.route_name,
    prediction: predictCrowdAdvanced(b.id, b.passengers_count || 0)
  }));
  res.json(predictions);
});

// ─── API: REAL-TIME SEAT AVAILABILITY ───
app.get('/api/buses/:id/seats', (req, res) => {
  const bus = stmts.getBusById.get(req.params.id);
  if (!bus) return res.status(404).json({ error: 'Bus not found' });
  res.json({
    busId: bus.id, registration: bus.registration,
    totalSeats: bus.capacity || 60,
    occupied: bus.passengers_count || 0,
    available: bus.seats_available || (bus.capacity - (bus.passengers_count || 0)),
    womenReserved: bus.women_reserved || 8,
    womenOccupied: Math.min(8, Math.round((bus.passengers_count || 0) * 0.15)),
    acAvailable: bus.ac_available || false,
    prediction: predictCrowdAdvanced(bus.id, bus.passengers_count || 0)
  });
});

// ─── API: AI SMART ROUTE RECOMMENDATION ───
app.post('/api/ai/route', (req, res) => {
  const { fromStopId, toStopId, mode } = req.body;
  if (!fromStopId || !toStopId) return res.status(400).json({ error: 'From and To required' });
  const result = smartRouteRecommend(fromStopId, toStopId, { mode: mode || 'balanced' });
  res.json(result);
});

// ─── API: WOMEN SAFETY MODE ───
app.post('/api/safety/start', authMiddleware, (req, res) => {
  const { busId, fromStopId, toStopId, shareWith } = req.body;
  const id = uuid();
  const now = new Date();
  stmts.createJourneyShare.run(id, req.user.id, busId, fromStopId, toStopId, now.toISOString(),
    shareWith || '', 0, 0, new Date(now.getTime() + 30*60000).toISOString());
  stmts.updateUserSafety.run(1, shareWith || '', req.user.id);
  // Send notification to emergency contacts
  const nid = uuid();
  stmts.createNotification.run(nid, req.user.id, 'safety_start', '🛡️ Safety Mode Active',
    'Your journey is being tracked live. Emergency contacts notified.', JSON.stringify({ journeyId: id }), '/safety');
  res.json({ success: true, journeyId: id, safetyMode: true });
});

app.post('/api/safety/update-location', authMiddleware, (req, res) => {
  const { journeyId, lat, lng } = req.body;
  stmts.updateJourneyLocation.run(lat, lng, journeyId);
...
app.post('/api/safety/sos', authMiddleware, (req, res) => {
  ...
  const id = uuid();
  stmts.createSos.run(id, req.user.id, busId, lat, lng, message || '🚨 EMERGENCY! Help needed!', 'sos_emergency');
  // Alert admin
...
app.post('/api/safety/end', authMiddleware, (req, res) => {
  const { journeyId } = req.body;
  stmts.endJourneyShare.run(journeyId);
  stmts.updateUserSafety.run(0, '', req.user.id);
  res.json({ success: true, safetyMode: false });
});

app.get('/api/safety/active', authMiddleware, (req, res) => {
  const journeys = stmts.getActiveJourneyShares.all(req.user.id);
  res.json(journeys);
});
...
app.post('/api/ai/chat', authMiddleware, (req, res) => {
  ...
  const result = aiAssistant(message, req.user.id, language || 'en');
  res.json(result);
});

app.post('/api/ai/voice', authMiddleware, (req, res) => {
  const { command, language } = req.body;
  if (!command) return res.status(400).json({ error: 'Command required' });
  const result = aiAssistant(command, req.user.id, language || 'en');
  stmts.saveVoiceCommand.run(req.user.id, command, language || 'en', result.intent, result.response, 0.85);
  res.json({ ...result, isVoice: true });
});

app.get('/api/ai/chat/history', authMiddleware, (req, res) => {
  res.json(stmts.getChatHistory.all(req.user.id));
});

// ─── API: SMART LANDMARK SEARCH ───
app.get('/api/stops/search/:query', (req, res) => {
  const query = `%${req.params.query}%`;
  const stops = stmts.searchStops.all(query, query, query);
  res.json(stops);
});

// ─── API: REWARDS ───
app.get('/api/rewards', authMiddleware, (req, res) => {
  const points = stmts.getRewardPoints.get(req.user.id);
  const history = stmts.getRewardHistory.all(req.user.id);
  res.json({ points: points?.total_reward_points || 0, history });
});

app.post('/api/rewards/earn', authMiddleware, (req, res) => {
  const { points, description } = req.body;
  const id = uuid();
  stmts.addRewardPoints.run(points || 10, req.user.id);
  stmts.createRewardTransaction.run(id, req.user.id, points || 10, 'earn', description || 'Trip completed');
  res.json({ success: true, pointsEarned: points || 10 });
});

// ─── API: COMMUNITY REPORTS ───
app.post('/api/community/report', authMiddleware, (req, res) => {
  const { busId, stopId, reportType, description, severity } = req.body;
  const id = uuid();
  stmts.createCommunityReport.run(id, req.user.id, busId, stopId, reportType, description, severity || 'normal');
  // Award points for reporting
  const rid = uuid();
  stmts.addRewardPoints.run(5, req.user.id);
  stmts.createRewardTransaction.run(rid, req.user.id, 5, 'earn', 'Community report reward');
  res.json({ success: true, reportId: id, pointsEarned: 5 });
});

app.get('/api/community/reports', (req, res) => {
  res.json(stmts.getCommunityReports.all());
});
...
app.get('/api/routes', (req, res) => res.json(stmts.getAllRoutes.all()));
...
app.get('/api/dashboard/stats', (req, res) => {
  ...
  const routes = stmts.getAllRoutes.all();
  const stops = stmts.getAllStops.all();
  const totalSeats = buses.reduce((s, b) => s + (b.seats_available || 0), 0);
  const womenSafeBuses = buses.filter(b => b.is_women_safe).length;

  res.json({
    totalBuses: total, totalRoutes: routes.length, totalStops: stops.length,
    avgOccupancy: avgPct, highCrowd: highCount, avgWait: avgWait,
    totalSeatsAvailable: totalSeats, womenSafeBuses,
...
io.on('connection', (socket) => {
  console.log('🔌 Neo Socket:', socket.id, socket.user?.role);
...
function seedDatabase() {
  ...
  const userPass = bcrypt.hashSync('user123', SALT_ROUNDS);
  const userId = uuid();
  stmts.createUser.run(userId, 'user@smartbus.com', userPass, 'John Passenger', '9876543211', 'passenger', 'en');
  const tamilPass = bcrypt.hashSync('tamil123', SALT_ROUNDS);
  stmts.createUser.run(uuid(), 'tamil@smartbus.com', tamilPass, 'Tamil User', '9876543212', 'passenger', 'ta');
  ...
  console.log(`   📍 ${stopIds.length} stops | 🚌 ${busRegs.length} buses | 🛣️ ${routeData.length} routes`);
}
...
// START
// ================================================================
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'smcd-1.html')));
...
server.listen(PORT, () => {
  ...
  console.log(`  👤  user@smartbus.com / user123`);
  console.log(`  👑  admin@smartbus.com / admin123`);
  console.log(`  🇮🇳  tamil@smartbus.com / tamil123 (தமிழ்)`);
  console.log('='.repeat(55) + '\n');
});
